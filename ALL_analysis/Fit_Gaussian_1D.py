...
def Fit_Gaussian_1D(h5file,show=True):
    ...
    for (cam_entry,j) in zip(cameras,range(len(cameras))):
            ...
            for (key,i) in zip(keys,range(len(keys))):
                ...
                out_x = imaging_analysis_lib_mod.do_fit_1D(n1D_x,x_vals,J_Gaussian1DModel,key+'_xfit_',ax[2*i+1,1])
                out_y = imaging_analysis_lib_mod.do_fit_1D(n1D_y,y_vals,J_Gaussian1DModel,key+'_yfit_',ax[2*i,0],invert=True)
...
if __name__ == '__main__':
    try:
        time_0 = time.time()
        import lyse
        show_warnings = lyse.SHOW_WARNINGS
        print('Show warnings:',show_warnings)
        run = lyse.Run(lyse.path)
        dict_save = None 
        with h5py.File(run.h5_path) as h5file:
            dict_save = Fit_Gaussian_1D(h5file)
