...
def _get_analysis_window_bounds(x_centers_um, params, mode_cfg):
    x_min_um = float(params.get('X_MIN_INTEGRATION', 0.0))
    x_max_um = float(params.get('X_MAX_INTEGRATION', x_min_um))
    if not isinstance(mode_cfg, dict):
        return x_min_um, x_max_um

    start_px = mode_cfg.get('analysis_window_start_px')
    size_px = mode_cfg.get('analysis_window_size_px')
    if start_px is None or size_px is None:
        return x_min_um, x_max_um

    try:
        start_px = int(start_px)
        size_px = int(size_px)
        if start_px < 0 or size_px <= 0 or x_centers_um.size == 0:
            return x_min_um, x_max_um

        start_px = min(start_px, len(x_centers_um) - 1)
        end_px = min(len(x_centers_um) - 1, start_px + size_px - 1)
        x_min_um = float(x_centers_um[start_px])
        x_max_um = float(x_centers_um[end_px])
        if x_max_um < x_min_um:
            x_min_um, x_max_um = x_max_um, x_min_um
        return x_min_um, x_max_um
    except Exception:
        return x_min_um, x_max_um
...
def waterfall_plot(df, seqs, scan, data_origin='show_ODs', constraints=None, average=False, title_labels=None, globals_in_title=None, params=None, plot_flags=None, mode_cfg=None):
    ...
    # Use a local copy of params so mode-specific analysis windows do not modify external state.
    params = dict(params)
    x_centers_um = np.arange(img_dims) * um_per_px
    x_min_um, x_max_um = _get_analysis_window_bounds(x_centers_um, params, mode_cfg if isinstance(mode_cfg, dict) else {})
    params['X_MIN_INTEGRATION'] = x_min_um
    params['X_MAX_INTEGRATION'] = x_max_um
    ...
    if scan == 'KZ_det_scan':
        ...
        # Determine analysis windows: either single window (xmin_det..xmax_det)
        # or divide the used region into N equal pixel windows when requested.
        windows = [(xmin_det, xmax_det)]
        if isinstance(mode_cfg, dict):
            divs = mode_cfg.get('analysis_window_divisions')
            if divs is not None:
                try:
                    n_div = int(divs)
                    if n_div > 1:
                        total_pts = (xmax_det - xmin_det + 1)
                        if total_pts > 0:
                            base = total_pts // n_div
                            rem = total_pts % n_div
                            cur = xmin_det
                            windows = []
                            for wi in range(n_div):
                                size = base + (1 if wi < rem else 0)
                                if size <= 0:
                                    continue
                                start = cur
                                end = min(cur + size - 1, xmax_det)
        ...
        # Build full ROI d and m signal lists for each shot.
        d_rois_det = []
        m_rois_det = []
        for i in range(len(det_x_raw)):
            ...
            if d_sig.size < 3 or xmax_det - xmin_det < 2:
                d_rois_det_full.append(np.array([], dtype=float))
                m_rois_det_full.append(np.array([], dtype=float))
            else:
                d_rois_det_full.append(d_sig[xmin_det:xmax_det + 1])
                m_rois_det_full.append(m_sig[xmin_det:xmax_det + 1])

        # Prepare threshold arrays: allow threshold scanning across the full ROI.
        ...
        n_shots = len(det_x_raw)
        thr_pos_per_shot = np.full(n_shots, float(deriv_thr_pos_det), dtype=float)
        thr_neg_per_shot = np.full(n_shots, float(deriv_thr_neg_det), dtype=float)
        print(
        ...
        if threshold_scan_det:
            for det_val, idx_det in zip(det_group_vals, det_group_indices):
                d_rois_group = [d_rois_det_full[ii] for ii in idx_det]
                ...
                # assign suggested thresholds for all shots in this det group
                thr_pos_per_shot[idx_det] = float(sug_pos)
                thr_neg_per_shot[idx_det] = float(sug_neg)
                print(
                    f"KZ_det_scan (full ROI) det={float(det_val):.6g}: +thr={sug_pos:.4f} ({mode_pos}), "
                    f"-thr={sug_neg:.4f} ({mode_neg})"
                )

        # Precompute full-ROI defect positions for each shot so moving-window analysis
        # can use the same defect count/position information across windows.
        shot_defect_x_pos_all = [np.array([], dtype=float) for _ in range(n_shots)]
        shot_defect_x_neg_all = [np.array([], dtype=float) for _ in range(n_shots)]
        shot_defect_x_corr_all = [np.array([], dtype=float) for _ in range(n_shots)]
        shot_defect_x_rej_all = [np.array([], dtype=float) for _ in range(n_shots)]
        shot_defect_x_all = [np.array([], dtype=float) for _ in range(n_shots)]

        for i in range(n_shots):
            d_roi = d_rois_det_full[i] if i < len(d_rois_det_full) else np.array([], dtype=float)
            m_roi = m_rois_det_full[i] if i < len(m_rois_det_full) else np.array([], dtype=float)
            if d_roi.size < 3:
                continue
            ...
            shot_defect_x_pos_all[i] = np.asarray(pos_x, dtype=float)
            shot_defect_x_neg_all[i] = np.asarray(neg_x, dtype=float)
            shot_defect_x_corr_all[i] = np.asarray(corrected_x, dtype=float)
            shot_defect_x_rej_all[i] = np.asarray([float(x_roi_um_det[int(idxr)]) for idxr in rejected_idx], dtype=float) if len(rejected_idx) > 0 else np.array([], dtype=float)
            shot_defect_x_all[i] = np.concatenate([
                shot_defect_x_pos_all[i],
                shot_defect_x_neg_all[i],
                shot_defect_x_corr_all[i],
            ]).astype(float)

        # Now run detection per window, collecting per-window results and plotting separately
        all_window_results = []
        for w, (s_px, e_px) in enumerate(windows):
            x_roi_um_win = x_centers_um_det[s_px:e_px + 1]
            det_counts_raw_w = []
            det_defect_x_w = []
            det_defect_y_w = []
            det_defect_x_pos_w = []
            ...
            det_defect_x_corr_w = []
            det_defect_y_corr_w = []
            det_domain_balance_raw_w = []

            for i in range(n_shots):
                d_roi = d_rois_det_full[i] if i < len(d_rois_det_full) else np.array([], dtype=float)
                m_roi = m_rois_det_full[i] if i < len(m_rois_det_full) else np.array([], dtype=float)
                if d_roi.size < 3:
                    det_counts_raw_w.append(0)
                    det_domain_balance_raw_w.append(np.nan)
                    continue
                ...
                det_domain_balance_raw_w.append(
                    _domain_balance_in_used_region(
                        m_roi,
                        x_roi_um_det,
                        defect_edges_x_shot,
                        float(x_centers_um_det[s_px]),
                        float(x_centers_um_det[e_px]),
                        start_sign=start_sign_shot,
                    )
                )

                # Assign the full-ROI defects to the current window only if they fall within the window bounds.
                w_left_um = float(x_centers_um_det[s_px])
                w_right_um = float(x_centers_um_det[e_px])
                pos_mask = (pos_x >= w_left_um) & (pos_x <= w_right_um)
                neg_mask = (neg_x >= w_left_um) & (neg_x <= w_right_um)
                corr_x = np.asarray(corrected_x, dtype=float)
                corr_mask = (corr_x >= w_left_um) & (corr_x <= w_right_um)
                rej_x = np.asarray([float(x_roi_um_det[int(idxr)]) for idxr in rejected_idx], dtype=float) if len(rejected_idx) > 0 else np.array([], dtype=float)
                rej_mask = (rej_x >= w_left_um) & (rej_x <= w_right_um)

                for x_thr in np.concatenate([pos_x[pos_mask], neg_x[neg_mask]]).astype(float):
                    det_defect_x_w.append(float(x_thr))
                    det_defect_y_w.append(float(i + 1))
                for x_p in pos_x[pos_mask]:
                    det_defect_x_pos_w.append(float(x_p))
                    det_defect_y_pos_w.append(float(i + 1))
                for x_n in neg_x[neg_mask]:
                    det_defect_x_neg_w.append(float(x_n))
                    det_defect_y_neg_w.append(float(i + 1))
                for x_r in rej_x[rej_mask]:
                    det_defect_x_rej_w.append(float(x_r))
                    det_defect_y_rej_w.append(float(i + 1))
                for xcorr in corr_x[corr_mask]:
                    det_defect_x_corr_w.append(float(xcorr))
                    det_defect_y_corr_w.append(float(i + 1))

                det_counts_raw_w.append(int(pos_mask.sum() + neg_mask.sum() + corr_mask.sum()))

            det_counts_raw_w = np.asarray(det_counts_raw_w, dtype=float)
            det_domain_balance_raw_w = np.asarray(det_domain_balance_raw_w, dtype=float)
            # Normalize by the used x-region length (between window xmin and xmax).
            used_region_um_w = float(abs(x_centers_um_det[e_px] - x_centers_um_det[s_px]))
            if used_region_um_w <= 0:
                used_region_um_w = 1.0
            det_norm_raw_w = det_counts_raw_w / used_region_um_w

            all_window_results.append({
                'window_index': w,
                'start_px': s_px,
                'end_px': e_px,
                'start_um': float(x_centers_um_det[s_px]),
                'end_um': float(x_centers_um_det[e_px]),
                'det_counts_raw': det_counts_raw_w,
                'det_domain_balance_raw': det_domain_balance_raw_w,
                'det_norm_raw': det_norm_raw_w,
                'det_defect_x': det_defect_x_w,
                'det_defect_y': det_defect_y_w,
                'det_defect_x_pos': det_defect_x_pos_w,
                'det_defect_y_pos': det_defect_y_pos_w,
                'det_defect_x_neg': det_defect_x_neg_w,
                'det_defect_y_neg': det_defect_y_neg_w,
                'det_defect_x_corr': det_defect_x_corr_w,
                'det_defect_y_corr': det_defect_y_corr_w,
                'det_defect_x_rej': det_defect_x_rej_w,
                'det_defect_y_rej': det_defect_y_rej_w,
            })

        # Moving-window analysis on ensemble magnetization and defect density per detuning.
        if isinstance(mode_cfg, dict) and bool(mode_cfg.get('moving_window_enabled', False)):
            ...
            if moving_window_width_um > 0 and moving_window_step_um > 0:
                ...
                while x_left + moving_window_width_um <= x_right + 1e-6:
                    window_lefts.append(float(x_left))
                    x_left += float(moving_window_step_um)
                if len(window_lefts) == 0:
                    window_lefts = [float(x_min_um_det)]
                window_lefts = np.asarray(window_lefts, dtype=float)
                window_rights = window_lefts + float(moving_window_width_um)
                window_centers = 0.5 * (window_lefts + window_rights)

                window_masks = [(x_centers_um_det >= wl) & (x_centers_um_det <= wr) for wl, wr in zip(window_lefts, window_rights)]
                ...
                for gi, shot_idx_group in enumerate(det_group_indices):
                    if len(shot_idx_group) == 0:
                        continue
                    group_m = M_full[np.asarray(shot_idx_group, dtype=int)]
                    for wi, mask in enumerate(window_masks):
                        ...
                        for shot_idx in shot_idx_group:
                            shot_defects = shot_defect_x_all[shot_idx]
                            densities.append(float(np.count_nonzero((shot_defects >= window_lefts[wi]) & (shot_defects <= window_rights[wi]))) / moving_window_width_um)
                        if len(densities) > 0:
                ...
                for gi in range(len(det_group_vals)):
                    ...
                    for wi in range(len(mag_vals)):
                        if not valid[wi]:
                            continue
                        if signs[wi] == 0:
                            zero_crossing_windows_by_det[gi].add(wi)
                        if wi > 0 and valid[wi - 1] and signs[wi] * signs[wi - 1] < 0:
                            zero_crossing_windows_by_det[gi].add(wi)
                            zero_crossing_windows_by_det[gi].add(wi - 1)

                fig_mwd, ax_mwd = plt.subplots(figsize=(9.0, 5.0), tight_layout=True)
                for gi, det_val in enumerate(det_group_vals):
                    color = window_colors[gi % len(window_colors)]
                    '''ax_mwd.errorbar(
                        window_centers,
                        window_defect_density[gi],
                        yerr=window_defect_sem[gi],
                        fmt='o-',
                        color=color,
                        ecolor=color,
                        elinewidth=1.2,
                ...
                ax_mwd.set_xlabel('x window center (µm)')
                ax_mwd.set_ylabel('Defect density (defects / µm)')
                ax_mwd.set_title('KZ_moving_window: average defect density vs window position')
                ax_mwd.grid(True, alpha=0.25)
                ax_mwd.legend(title='det', fontsize='small', loc='best')

                fig_mwm, ax_mwm = plt.subplots(figsize=(9.0, 5.0), tight_layout=True)
                for gi, det_val in enumerate(det_group_vals):
                    color = window_colors[gi % len(window_colors)]
                    ax_mwm.errorbar(
                        window_centers,
                        window_mag_avg[gi],
                        yerr=window_mag_sem[gi],
                        fmt='s-',
                        color=color,
                        ecolor=color,
                        elinewidth=1.2,
                ...
                ax_mwm.set_xlabel('x window center (µm)')
                ax_mwm.set_ylabel('Ensemble mean magnetization')
                ax_mwm.set_title('KZ_moving_window: ensemble magnetization vs window position')
                ax_mwm.grid(True, alpha=0.25)
                ax_mwm.legend(title='det', fontsize='small', loc='best')

        # If only one window, keep backward-compatible variable names for downstream plotting.
        if len(all_window_results) == 1:
            res = all_window_results[0]
            det_counts_raw = res['det_counts_raw']
            det_domain_balance_raw = res['det_domain_balance_raw']
            det_norm_raw = res['det_norm_raw']
            det_defect_x = res['det_defect_x']
            det_defect_y = res['det_defect_y']
            det_defect_x_pos = res['det_defect_x_pos']
            det_defect_y_pos = res['det_defect_y_pos']
            det_defect_x_neg = res['det_defect_x_neg']
            det_defect_y_neg = res['det_defect_y_neg']
            det_defect_x_rej = res['det_defect_x_rej']
            det_defect_y_rej = res['det_defect_y_rej']
            det_defect_x_corr = res['det_defect_x_corr']
            det_defect_y_corr = res['det_defect_y_corr']
            det_x_raw_orig = det_x_raw  # Save original for per-window plotting
        else:
            # Multi-window: concatenate results from all windows for unified plotting
            det_x_raw_orig = det_x_raw.copy()  # Save original before repeating
            det_counts_raw = np.concatenate([res['det_counts_raw'] for res in all_window_results])
            det_domain_balance_raw = np.concatenate([res['det_domain_balance_raw'] for res in all_window_results])
            det_norm_raw = np.concatenate([res['det_norm_raw'] for res in all_window_results])
            # Repeat detuning values to match the number of windows (each shot has n_windows results)
            det_x_raw = np.repeat(det_x_raw, n_windows)
            det_defect_x = [x for res in all_window_results for x in res['det_defect_x']]
            det_defect_y = [y for res in all_window_results for y in res['det_defect_y']]
            det_defect_x_pos = [x for res in all_window_results for x in res['det_defect_x_pos']]
            det_defect_y_pos = [y for res in all_window_results for y in res['det_defect_y_pos']]
            det_defect_x_neg = [x for res in all_window_results for x in res['det_defect_x_neg']]
            det_defect_y_neg = [y for res in all_window_results for y in res['det_defect_y_neg']]
            det_defect_x_rej = [x for res in all_window_results for x in res['det_defect_x_rej']]
            det_defect_y_rej = [y for res in all_window_results for y in res['det_defect_y_rej']]
            det_defect_x_corr = [x for res in all_window_results for x in res['det_defect_x_corr']]
            det_defect_y_corr = [y for res in all_window_results for y in res['det_defect_y_corr']]
        
        # Compute total used-region length (um) for summary printing.
        try:
            if len(all_window_results) == 1:
                ...
                for w in all_window_results:
                    used_region_um += float(abs(w['end_um'] - w['start_um']))
        except Exception:
            used_region_um = float(abs(x_max_um_det - x_min_um_det))

        # Create per-window defect plots if multiple windows are used.
        if n_windows > 1:
            for w_idx, w_res in enumerate(all_window_results):
                w_det_norm = w_res['det_norm_raw']
                w_det_dom = w_res['det_domain_balance_raw']
                w_det_counts = w_res['det_counts_raw']
                w_start_um = w_res['start_um']
                w_end_um = w_res['end_um']
                
                if w_det_norm.size > 0:
                    # Group by detuning for this window
                    w_det_mean = []
                    w_det_sem = []
                    w_det_n = []
                    w_dom_mean = []
                    ...
                    for idx_det in det_group_indices:
                        vals = w_det_norm[idx_det]
                        w_det_mean.append(float(np.mean(vals)) if vals.size > 0 else 0.0)
                        if vals.size > 1:
                            w_det_sem.append(float(np.std(vals, ddof=1) / np.sqrt(vals.size)))
                        else:
                            w_det_sem.append(0.0)
                        w_det_n.append(int(vals.size))
                        # Domain balance for this detuning group
                        dom_vals = w_det_dom[idx_det]
                        w_dom_mean.append(float(np.mean(dom_vals)) if dom_vals.size > 0 else 0.0)
                        if dom_vals.size > 1:
                            w_dom_sem.append(float(np.std(dom_vals, ddof=1) / np.sqrt(dom_vals.size)))
                        else:
                            w_dom_sem.append(0.0)
                    
                    w_det_mean = np.asarray(w_det_mean, dtype=float)
                    w_det_sem = np.asarray(w_det_sem, dtype=float)
                    w_dom_mean = np.asarray(w_dom_mean, dtype=float)
                    w_dom_sem = np.asarray(w_dom_sem, dtype=float)
                    
                    # Plot per-window defect density and domain balance
                    fig_w, ax_w = plt.subplots(figsize=(9.0, 5.5), tight_layout=True)
                    
                    # Left y-axis: defect density
                    ax_w.scatter(det_x_raw_orig, w_det_norm, s=18, color='0.6', alpha=0.65, label='single shot (defects)')
                    ax_w.errorbar(
                        det_group_vals,
                        w_det_mean,
                        yerr=w_det_sem,
                        ...
                        markersize=5,
                        linewidth=1.5,
                        label='mean ± SEM (defects)',
                    )
                    ax_w.set_xlabel('ARPKZ_final_set_field')
                    ax_w.set_ylabel(r'Defects / used region ($\mu m^{-1}$)', color='tab:blue')
                    ax_w.tick_params(axis='y', labelcolor='tab:blue')
                    
                    # Right y-axis: domain balance
                    ax_dom = ax_w.twinx()
                    ax_dom.errorbar(
                        det_group_vals,
                        w_dom_mean,
                    ...
                    ax_dom.set_ylabel('Domain Balance', color='tab:red')
                    ax_dom.tick_params(axis='y', labelcolor='tab:red')
                    ax_dom.set_ylim(-1.1, 1.1)
                    
                    ax_w.set_title(f'Defects & Domain Balance vs Detuning: Window {w_idx + 1} (x={w_start_um:.1f}–{w_end_um:.1f} µm, seqs: {seqs})')
                    ax_w.grid(True, alpha=0.25)
                    
                    # Combined legend
                    lines1, labels1 = ax_w.get_legend_handles_labels()
                    lines2, labels2 = ax_dom.get_legend_handles_labels()
                    ax_w.legend(lines1 + lines2, labels1 + labels2, loc='best', fontsize=9)
                    
                    print(f"\nKZ_det_scan Window {w_idx + 1} (x={w_start_um:.1f}–{w_end_um:.1f} µm):")
                    for xv, m, s, n, d, ds in zip(det_group_vals, w_det_mean, w_det_sem, w_det_n, w_dom_mean, w_dom_sem):
                        print(f"  det={xv:.6g}: defects/region={m:.5f} ± {s:.5f} um^-1, domain_balance={d:.4f} ± {ds:.4f}, shots={n}")
        ...
        if det_counts_raw.size > 0:
            ...
            def _draw_zero_crossing_lines(ax):
                if len(det_group_vals) == 0 or len(M_full) == 0:
                    return
                for gi, det_val in enumerate(det_group_vals):
                    if len(zero_crossing_windows_by_det[gi]) == 0:
                        continue
                    ...
                    zero_windows = sorted(zero_crossing_windows_by_det[gi])
                    x_min_zc = float(window_lefts[zero_windows[0]])
                    x_max_zc = float(window_rights[zero_windows[-1]])
                    color = window_colors[gi % len(window_colors)]
                    ax.plot(
                        [x_min_zc, x_min_zc],
                        [y_min_det, y_max_det],
                        ...
                        alpha=0.75,
                        zorder=4,
                        label=f'{det_val:.6g} zero crossing',
                    )
                    ax.plot(
                        [x_max_zc, x_max_zc],
                        [y_min_det, y_max_det],
            ...
            # Add window boundary lines if multiple windows are used
            if n_windows > 1:
                for w_res in all_window_results:
                    w_start_um = w_res['start_um']
                    w_end_um = w_res['end_um']
                    ax_raw.axvline(x=float(w_start_um), color='orange', linestyle=':', linewidth=1.8, alpha=0.8)
                    ax_raw.axvline(x=float(w_end_um), color='orange', linestyle=':', linewidth=1.8, alpha=0.8)
            ...
            # Compute a reconstructed binary magnetization matrix (±1) from detected walls
            def _compute_reconstructed_matrix():
                ...
                for shot_idx in range(len(M_full)):
                    ...
                    sign_from_defect = _infer_start_sign_from_signed_walls(
                        np.asarray(det_x_pos_arr[np.isclose(det_y_pos_arr, y_shot, atol=1e-9, rtol=0.0)], dtype=float),
                        np.asarray(det_x_neg_arr[np.isclose(det_y_neg_arr, y_shot, atol=1e-9, rtol=0.0)], dtype=float),
                    )
                    if sign_from_defect is not None:
                        sign_val = float(sign_from_defect)
                    else:
                    ...
                    used_mask = (x_axis >= float(x_min_um_det)) & (x_axis <= float(x_max_um_det))
                    row_bin = np.where(used_mask, row_bin, 0.0)
                    rec[shot_idx, :] = row_bin
                return rec

            M_rec = _compute_reconstructed_matrix()

            # Reconstructed magnetization waterfall (binary ±1)
