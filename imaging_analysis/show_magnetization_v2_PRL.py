...
# Rescaling factor for img_m2 (accounts for expansion due to longer imaging time)
# Set to 1.0 for no rescaling, < 1.0 to contract, > 1.0 to expand
RESCALE_FACTOR_IMG_M2 = 1/1.78  #Obtained from ratio of Thomas-Fermi fit radii for m1 and m2 (see separate notebook for details)
...
# =============================================================================
try:
    spec = importlib.util.spec_from_file_location("imaging_analysis_lib.camera_settings", f"{ANALYSISLIB_V2}/imaging_analysis_lib/camera_settings.py")
    camera_settings = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(camera_settings)

    spec = importlib.util.spec_from_file_location("imaging_analysis_lib.main", f"{ANALYSISLIB_V2}/imaging_analysis_lib/main.py")
    imaging_analysis_lib_mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(imaging_analysis_lib_mod)

    spec = importlib.util.spec_from_file_location("general_lib.main", f"{ANALYSISLIB_V2}/general_lib/main.py")
    general_lib_mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(general_lib_mod)
    
    spec = importlib.util.spec_from_file_location("defect_finding_lib", f"{ANALYSISLIB_V2}/waterfall/defect_finding_lib.py")
...
def load_affine_coefficients():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "waterfall_config",
            f"{ANALYSISLIB_V2}/waterfall/waterfall_config.py"
        )
        ...
        print(f"WARNING: Failed to load affine coefficients from {ANALYSISLIB_V2}/waterfall/waterfall_config.py: {e}")
        print(f"DEBUG: Falling back to identity transform")
...
def run_full_defect_analysis(m_profile, x_axis_um, x_min_um, x_max_um, Z=None, y_roi=None, title="Defect Analysis"):
    ...
    # Create figure with all algorithm steps (fifth plot at half height)
    fig = plt.figure(figsize=(3.39, 10.2))
    gs = fig.add_gridspec(5, 1,
    height_ratios=[1, 1, 1, 1, 0.5],
    hspace=0.3)

    axes = [fig.add_subplot(gs[i]) for i in range(5)]
    # Link x-axes to first subplot
    for i in range(1, 5):   
    ...
    def shade_domains(ax, x_roi_um, boundaries, signs, y_min, y_max):
        ...
        for (x_start, x_end), sign in zip(boundaries, signs):
            ...
            ax.fill_between(x_roi_um, y_min, y_max, where=domain_mask, alpha=0.3, color=color)
    ...
    if Z is not None:
        ...
        x_rel_um = x_roi_um - x_roi_um[0]   # va da 0 a L
        domain_boundaries_rel = [(x0 - x_roi_um[0], x1 - x_roi_um[0]) for (x0, x1) in domain_boundaries]

        #im = ax.imshow(Z_roi, vmin=-1, vmax=1, cmap='RdBu', aspect='auto', extent=[x_roi_um_array[0], x_roi_um_array[-1], y_max_um, 0])
        im = ax.imshow(
            Z_roi,
            vmin=-1, vmax=1,
            cmap='RdBu',
            aspect='auto',
            extent=[x_rel_um[0], x_rel_um[-1], y_max_um, 0]
        )

        ax.set_xlim(x_rel_um[0], x_rel_um[-1])

        # --- PRL-style colorbar: does NOT resize the axes ---
        # Colorbar
        ...
        '''for (x_start, x_end), sign in zip(domain_boundaries, domain_signs):
            if x_start > x_roi_um_array[0] and x_start < x_roi_um_array[-1]:
                ax.axvline(x_start, color='k', linestyle='-', linewidth=1, alpha=0.6)
            if x_end > x_roi_um_array[0] and x_end < x_roi_um_array[-1]:
                ax.axvline(x_end, color='k', linestyle='-', linewidth=1, alpha=0.6)'''
        ...
        ax.axvline(0, color='g', linestyle=':', linewidth=2, alpha=0.7)
        ax.axvline(x_rel_um[-1], color='g', linestyle=':', linewidth=2, alpha=0.7)
        ...
        ax.set_xlim(0, x_rel_um[-1])

        ax.set_ylim(30, 100)  # Focus on central region where atoms are
        ax.set_ylabel('y (µm)', fontsize=11)
        #ax.set_title('2D magnetization', fontsize=10)
        #ax.set_title('2D magnetization', fontsize=10, pad=30)
    else:
        ax.text(0.5, 0.5, 'No 2D data provided', ha='center', va='center', transform=ax.transAxes)
        #ax.set_title('Step 1: 2D Magnetization (no data)', fontsize=10)
    
    # Step 2: Raw and smoothed rescaled z1d
    ax = axes[1]
    m_smooth = defect_finding_lib.gaussian_filter1d(m_roi, sigma=defect_params.get('gaussian_sigma', 0.))
    #shade_domains(ax, x_roi_um, domain_boundaries, domain_signs, m_roi.min() - 0.1, m_roi.max() + 0.1)
    ax.plot(x_rel_um - x_rel_um[0], m_roi, '-', color='k', linewidth=1.5, alpha=0.6, label='Raw rescaled z1d')
    #ax.plot(x_roi_um - x_roi_um[0], m_smooth, 'r-', linewidth=2.5, label='Gaussian smoothed (σ={:.2f})'.format(defect_params.get('gaussian_sigma', 0.3)))
    ...
    #ax.set_title('Renormalized 1D magnetization', fontsize=10)
    ax.grid(True, alpha=0.3)
    #ax.tick_params(labelsize=8)

    # Step 3: Peak-step filter metric
    ax = axes[2]
    window_px = int(defect_params.get('peak_step_filter_window_px', 4))
    thr_delta = float(defect_params.get('peak_step_filter_min_abs_delta_m', 0.7))
    step_filter = np.full_like(m_roi, np.nan, dtype=float)
    for i in range(len(m_roi)):
        l0 = i - window_px
        l1 = i
        r0 = i + 1
        r1 = i + 1 + window_px
        if l0 >= 0 and r1 <= len(m_roi):
            avg_left = np.mean(m_roi[l0:l1])
            avg_right = np.mean(m_roi[r0:r1])
            #step_filter[i] = abs(avg_right - avg_left)
            step_filter[i] = avg_right - avg_left

    ax.plot(x_roi_um - x_roi_um[0], step_filter, '-', color='k', linewidth=1.5, alpha=0.6, label=f'Peak-step filter |ΔZ| (window={window_px} px)')
    ax.axhline(thr_delta, color='tab:red', linestyle='--', linewidth=1.5, alpha=0.8,
               label=f'Rejection threshold {thr_delta:.2f}')
    ax.axvline(x_min_um, color='g', linestyle=':', linewidth=2, alpha=0.7, label='ROI boundaries')
    ax.axvline(x_max_um, color='g', linestyle=':', linewidth=2, alpha=0.7)
    ax.set_ylabel('ΔZ', fontsize=11)
    #ax.set_title(f'Average magnetization jump over {window_px} μm', fontsize=10)
    ax.grid(True, alpha=0.3)
    #ax.tick_params(labelsize=8)
    #ax.legend(fontsize=7, loc='upper right')
    
    # Step 4: Derivative peaks (renumbered to axes[3])
    ax = axes[3]
    ax.plot(x_roi_um - x_roi_um[0], d_roi, 'k-', linewidth=1.5, alpha=0.6, label='Derivative')
    
    # Threshold lines
    thr_pos = defect_params.get('derivative_threshold_pos', 0.1)
    thr_neg = defect_params.get('derivative_threshold_neg', -0.1)
    ax.axhline(thr_pos, color='tab:blue', linestyle='--', linewidth=1.5, alpha=0.7, label=f'Pos threshold: {thr_pos:.4f}')
    ax.axhline(thr_neg, color='tab:red', linestyle='--', linewidth=1.5, alpha=0.7, label=f'Neg threshold: {thr_neg:.4f}')
    ...
    if len(threshold_x_pos) > 0:
        ...
        ax.scatter(threshold_x_pos - x_roi_um[0], d_pos_vals, color='black', s=30, marker='^', 
                  linewidths=1.5, edgecolors='black', zorder=5, label=f'Pos peaks ({len(threshold_x_pos)})')
    if len(threshold_x_neg) > 0:
        d_neg_vals = np.interp(threshold_x_neg, x_roi_um, d_roi)
        ax.scatter(threshold_x_neg - x_roi_um[0], d_neg_vals, color='black', s=30, marker='v', 
                  linewidths=2, edgecolors='black', zorder=5, label=f'Neg peaks ({len(threshold_x_neg)})')
    

    '''if len(threshold_x_pos) > 0:
        d_pos_vals = np.interp(threshold_x_pos, x_roi_um, d_roi)
        ax.scatter(threshold_x_pos, d_pos_vals, color='tab:blue', s=50, marker='^', 
                  linewidths=1.5, edgecolors='darkblue', zorder=5, label=f'Pos peaks ({len(threshold_x_pos)})')
    if len(threshold_x_neg) > 0:
        d_neg_vals = np.interp(threshold_x_neg, x_roi_um, d_roi)
        ax.scatter(threshold_x_neg, d_neg_vals, color='tab:red', s=50, marker='v', 
    ...
    if len(rejected_x) > 0:
        ...
        ax.scatter(rejected_x - x_roi_um[0], d_rej_vals, color='orange', s=120, marker='x', 
                  linewidths=3, alpha=0.7, zorder=4, label=f'Rejected peaks ({len(rejected_x)})')
    
    ax.set_ylabel('dZ/dx', fontsize=11)
    #ax.set_title('Magnetization derivative', fontsize=10)
    ax.grid(True, alpha=0.3)
    #ax.tick_params(labelsize=8)
    ...
    ax = axes[4]
    shade_domains(ax, x_roi_um - x_roi_um[0], domain_boundaries_rel, domain_signs, -1.2, 1.2)
    ...
    if len(all_peaks) > 0:
        for peak in all_peaks:
            ax.axvline(peak - x_roi_um[0], color='gray', linestyle=':', alpha=1., linewidth=0.8)
    
    #ax.set_ylabel('Z', fontsize=10)
    #ax.set_xlabel('X (μm)', fontsize=10)
    #ax.set_title(f'Reconstructed magnetization', fontsize=10)
    ...
    #fig.subplots_adjust(left=0.18, right=0.97, top=0.96, bottom=0.08)
    
    return results, fig

def get_bimodal_center_m1_y(h5file, cam, key='PTAI_m1', invert=True):
    """
    Bimodal fit on m1 integrated along y.
    Returns BEC center y0 and full fit object.
    """
    ...
    # Extract 2D image
    n2D, _, _, x_vals, y_vals = imaging_analysis_lib_mod.extract_images_parts(
        dict_images, cam, key
    )

    # ROI
    roi = cam['roi_integration']
    x_min, x_max = roi[1].start, roi[1].stop
    print(f"DEBUG: Using ROI x indices [{x_min}:{x_max}] for bimodal fit along y")

    # 1D profile along y (integrate over x)
    n1D_y = np.sum(n2D[:, x_min:x_max], axis=1) * cam['px_size']

    fig, ax = plt.subplots()
    ax.plot(y_vals, n1D_y)
    

    # Fit bimodal model
    out = imaging_analysis_lib_mod.do_fit_1D(
        n1D_y,
        y_vals,
        J_BimodalBose1DModel2Centers,
        'bimodal_m1_yfit_',
        ax=ax,
        invert=False
    )
    ...
    if out is None:
        print("DEBUG: do_fit_1D returned None - fit may have failed")
        return None

    params = out.params.valuesdict()
    print(f"DEBUG: Fit parameters: {params}")

    # Extract BEC center robustly
    if 'bimodal_m1_yfit_c_tf' in params:
        y0 = int(params['bimodal_m1_yfit_c_tf']/cam['px_size'])  # Convert from pixels to micrometers
    else:
        print("DEBUG: Cannot find BEC center parameter in fit output")
        return None
...
def compute_magnetization_and_density_rescale(img_m1,img_m2,x0,lambda_x):
    """
    Computes total density (D) and magnetization (Z),
    rescaling img_m1 along x by factor lambda_x.

    Parameters
    ----------
    img_m1 : 2D ndarray
        Density image of component m1 (earlier TOF)
    img_m2 : 2D ndarray
        Density image of component m2 (later TOF)
    x0 : float
        Condensate center along x (in pixel units),
        obtained from Thomas-Fermi fit
    lambda_x : float
        Expansion factor along x
    """
    ...
    interp_m1 = RegularGridInterpolator(
        (x, y),
        img_m1,
        bounds_error=False,
        fill_value=0
    ...
    # Rescale about the TF center
    coords = np.stack(
    [(X - x0) / lambda_x + x0, Y],
    axis=-1)

    # Jacobian for density conservation
    img_m1_rescaled = (1 / lambda_x) * interp_m1(coords)

    D_tot = img_m1_rescaled + img_m2

    with np.errstate(divide='ignore', invalid='ignore'):
        Z = (img_m2 - img_m1_rescaled) / D_tot

    return D_tot, Z, img_m1_rescaled
...
def plot_smoothing_analysis(g2, fit_result, exp_fit_result, smooth_sweep_data, title, pre_filter_sigma):
    ...
    print("DEBUG: Saving figure to /home/diego/Scrivania/figure2.png")
    plt.savefig(f"/home/diego/Scrivania/figure2.png", dpi=300)
    return fig
...
def plot_magnetization(images_ODlog, img_m1, img_m2, Z, D, y_roi, title, show=True):
    ...
    """
    # Validate inputs
    if images_ODlog is None or Z is None or D is None or y_roi is None:
        print(f"DEBUG: plot_magnetization received None values - images_ODlog: {images_ODlog is not None}, Z: {Z is not None}, D: {D is not None}, y_roi: {y_roi is not None}")
        return None
    
    y_start, y_end = y_roi
    

    fig, ax = plt.subplots(4, 1, figsize=(3.39, 8), sharex=True)

    # Plot m1
    # Note: aspect='auto' fixes the "locking" issue when zooming with shared axes
    ax[1].imshow(images_ODlog['PTAI_m1'], vmin=0, vmax=1.5e14, cmap='hot', aspect='auto')
    ax[1].set_title('Optical density  n1')
    ax[1].set_ylim(95,30)
    ax[1].set_ylabel('y (μm)', fontsize=11)
    ax[1].set_xlim(750,1300)
    
    # Plot m2
    ax[0].imshow(images_ODlog['PTAI_m2'], vmin=0, vmax=1.5e14, cmap='hot', aspect='auto')
    ax[0].set_title('Optical density  n2')
    ax[0].set_ylim(95,30)
    ax[0].set_ylabel('y (μm)', fontsize=11)
    ax[0].set_xlim(750,1300)    

    # Plot Z
    ax[3].imshow(Z, vmin=-1, vmax=1, cmap='RdBu', aspect='auto')
    ax[3].set_title('Z = (n2-n1)/(n1+n2)')
    ax[3].set_ylabel('y (μm)', fontsize=11)
    ax[3].set_xlabel('x (μm)', fontsize=11)
    ax[3].set_ylim(95,30)
    ax[3].set_xlim(750,1300)
    
    # Plot D (n1 + n2)
    # Using ODlog sum for visualization as in original code
    d_visual = images_ODlog['PTAI_m1'] + images_ODlog['PTAI_m2']
    ax[2].imshow(d_visual, vmin=0, vmax=1.5e14, cmap='hot', aspect='auto')
    ax[2].set_title('Optical density: n1 + n2')
    ax[2].set_ylim(95,30)
    ax[2].set_xlim(750,1300)
    ax[2].set_ylabel('y (μm)', fontsize=11)
...
def plot_z1d_derivative(
    ...
    try:
        ...
        if defect_result is not None:
            ...
            # Save the defect analysis figure to home directory
            import os
            home_dir = os.path.expanduser("~")
            h5_base = os.path.splitext(os.path.basename(h5file.filename))[0]
            fig_filename = os.path.join(home_dir, f"{h5_base}_defect_analysis_figure.png")
            fig_defect_analysis.savefig(fig_filename, dpi=300, bbox_inches='tight')
            print(f"Saved defect analysis figure to: {fig_filename}")
    except Exception as e:
...
def thrshold_scan_analysis(thr_values, counts, current_thr, title, save_path=None):
    """
    Analyzes the count vs threshold curve to suggest an optimal threshold based on plateaus.
    Also generates a plot of the count vs threshold curve with annotations.
    """
    suggested_thr, plateau_range, mode = _suggest_threshold_from_plateau(
        thr_values, counts, current_thr, PLATEAU_DELTA_DEFECTS, PLATEAU_MIN_POINTS
    )
    
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(thr_values, counts, 'o-', label='Defect Count')
    ax.axvline(suggested_thr, color='r', linestyle='--', label=f'Suggested Thr: {suggested_thr:.4f} ({mode})')
    
    if plateau_range is not None:
        ax.axvspan(plateau_range[0], plateau_range[1], color='red', alpha=0.2, label='Suggested Plateau Range')
    
    ax.set_title(title)
    ax.set_xlabel('Derivative Threshold')
    ax.set_ylabel('Number of Detected Defects')
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def show_magnetization_v2(h5file, show=True):
    ...
    try:
        ...
        # Also plot the same quantities but using rescaled m1 (accounts for expansion)
        try:
            cam = CAMERAS[camera_name]
            x0 = get_bimodal_center_m1_y(h5file, cam, key='PTAI_m1', invert=True)
            print("BEC center:", x0)

            x0_est = x0
            # use RESCALE_FACTOR_IMG_M2 for lambda (default 1.0)
            D_resc, Z_resc, img_m1_resc_vis = compute_magnetization_and_density_rescale(img_m1, img_m2, x0=x0_est, lambda_x=RESCALE_FACTOR_IMG_M2)
            # create a simple images_ODlog equivalent for visualization using original ODlog but replace m1 with rescaled visual
            images_ODlog_resc = images_ODlog.copy()
            # for display purposes use D_resc and Z_resc (but preserve original ODlog keys)
            # create a visual rescaled m1 by subtracting img_m2 from D_resc (may contain floats)
            images_ODlog_resc['PTAI_m1'] = img_m1_resc_vis
            # call plotting routine with rescaled m1
            print("DEBUG: Plotting rescaled magnetization...")
            plot_magnetization(images_ODlog_resc, img_m1_resc_vis, img_m2, Z_resc, D_resc, y_roi, title = None, show=show)
            print("DEBUG: Rescaled main plot finished.")

        except Exception as e:
            print(f"DEBUG: Failed to plot rescaled magnetization: {e}")
            traceback.print_exc()
        ...
        if True:
            ...
            fig_deriv, nb_zero_crossings, zero_crossing_indices, z1d_avg, z1d_std, z1d_domain_avg = plot_z1d_derivative(Z_resc, img_m1_resc_vis, img_m2, y_roi, title, optimize_thresholds=False)
